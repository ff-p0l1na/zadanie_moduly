...
for n in range(ARG_ZMIANY, len(sys.argv)):  # arg zmiany to co do zasady bedzie 3, a koniec to n, stad dwukropek
    ...
    wybrana_kolumna = int(komorka_i_tresc[0])  # to tylko zamysl usera, jeszcze nie idx z csv
    wybrany_wiersz = int(komorka_i_tresc[1])  # same here, to jeszcze nie idx z csv
    tresc = komorka_i_tresc[2] # to sie nie zmieni
# przerobienie wybranych wartosci na indexy listy
    zmieniany_index_kolumna = int(wybrana_kolumna - 1)  # same but different, z userowego na kompowy
    zmieniany_index_wiersz = int(wybrany_wiersz - 1)  # same but different, z userowego na kompowy
    wartosci_do_zmiany.append([zmieniany_index_kolumna, zmieniany_index_wiersz, tresc]) # wdz = [[1,2,tresc], [2,3,trresc]]
#
...
# "zmieniany_index_kolumna" bedzie miec wartosc pierwszego el. listy "wartosci do zmiany" and so on
    ...
    zawartosc_pliku[zmieniany_index_wiersz][zmieniany_index_kolumna] = tresc
# zawartosc_pliku przyklad:
# [[a,b,c], <- a to zp[1][1]
# [d,e,f,], <- f to zp[2][3]
# [g,h,i]]
